...
const nodes = [
  {
    ...
    delay: 'delay-0',
    position: 'top-[16%] left-[24%] md:top-[20%] md:left-[22%] -translate-x-1/2 -translate-y-1/2',
  },
  {
    path: '/pelucias',
    ...
    delay: 'delay-75',
    position: 'top-[26%] left-[76%] md:top-[20%] md:left-[78%] -translate-x-1/2 -translate-y-1/2',
  },
  {
    path: '/contos',
    image:
    ...
    delay: 'delay-100',
    position: 'top-[76%] left-[24%] md:top-[75%] md:left-[22%] -translate-x-1/2 -translate-y-1/2',
  },
  {
    path: '/trofeus',
    ...
    delay: 'delay-150',
    position: 'top-[86%] left-[76%] md:top-[75%] md:left-[78%] -translate-x-1/2 -translate-y-1/2',
  },
  {
    path: '/missoes',
    ...
    <div className="min-h-[calc(100vh-80px)] w-full relative flex flex-col items-center justify-start md:justify-center p-2 sm:p-4 overflow-x-hidden pt-8 md:pt-4 pb-16 md:pb-4">
      ...
      <div className="relative w-full max-w-5xl h-[700px] sm:h-[750px] md:h-[800px] shrink-0 my-auto">
        ...
        {urgentTasks.length > 0 && (
          <Link
            to="/missoes"
            className="absolute -top-4 md:top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 md:gap-3 bg-gradient-to-b from-red-500 to-red-700 text-white px-4 py-2 md:px-6 md:py-3 rounded-full shadow-[0_10px_30px_rgba(220,38,38,0.5)] border-2 md:border-4 border-yellow-400 hover:scale-105 hover:-translate-y-1 transition-all duration-300 group animate-fade-in-down w-11/12 max-w-[320px] md:max-w-none md:w-auto justify-center"
          >
            <div className="bg-yellow-400 text-red-700 rounded-full p-1 md:p-1.5 shadow-inner shrink-0">
              <AlertCircle className="w-4 h-4 md:w-5 md:h-5 animate-pulse" strokeWidth={3} />
            </div>
            <span className="font-display font-black text-xs md:text-base uppercase tracking-wider text-shadow-sm drop-shadow-md whitespace-nowrap">
              {urgentTasks.length}{' '}
              {urgentTasks.length === 1 ? 'Tarefa Urgente!' : 'Tarefas Urgentes!'}
            </span>
            <ChevronRight
              className="w-4 h-4 md:w-5 md:h-5 group-hover:translate-x-1 transition-transform text-yellow-300 shrink-0"
              strokeWidth={3}
            />
          </Link>
        )}
        ...
        {nodes.map((node, i) => (
          ...
          >
            <div
              className={cn(
                'w-24 h-24 md:w-32 md:h-32 rounded-full shadow-xl flex items-center justify-center relative overflow-hidden border-[3px] md:border-4 bg-white',
                node.color,
              )}
            >
              <img
            ...
            <div className="bg-white/95 backdrop-blur-md px-4 md:px-5 py-1.5 md:py-2 rounded-full shadow-lg border-2 border-orange-200 group-hover:border-orange-400 transition-colors">
              <span className="font-display font-black text-sm md:text-lg text-orange-900 text-center whitespace-nowrap uppercase tracking-wider">
                {node.title}
              </span>
            </div>
          </Link>
